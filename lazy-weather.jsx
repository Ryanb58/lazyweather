...
function comparePhrase(deltaC, mode) {
  ...
  const fixedSuffix =
    emphasis === "ABOUT THE SAME" && mode === "today" ? "as today" : suffix;
...
async function fetchWeather(lat, lon) {
  ...
  console.log("[lazy-weather] fetching:", url);
  let r;
  try {
    r = await fetch(url);
  } catch (e) {
    console.error("[lazy-weather] network error:", e);
    throw new Error(`network: ${e.message || e}`);
  }
  if (!r.ok) throw new Error(`HTTP ${r.status}`);
  const j = await r.json();
  console.log("[lazy-weather] got data:", j);
  ...
  const tmAvg =
    j.daily.temperature_2m_max[2] != null
      ? avg(j.daily.temperature_2m_max[2], j.daily.temperature_2m_min[2])
      : tAvg;
...
export default function LazyWeather() {
  ...
  const resolveLocation = useCallback(async (forceGeo = false) => {
    ...
    try {
      ...
      try {
        setCity(await reverseGeocode(lat, lon));
      } catch {
        setCity("Here");
      }
    } catch (e) {
      if (forceGeo) setErr("Couldn't get your location. Check browser permissions.");
      ...
      <div className="lw max-w-md mx-auto h-full flex flex-col relative">
        ...
        >
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div
                className="absolute right-0 mt-2 rounded-2xl bg-[#1a1a1a] p-2 w-52 text-sm z-20"
              ...
              >
                ...
                <button
                  ...
                  onClick={() => { setUnit("C"); setMenuOpen(false); }}
                >
                  Celsius
                </button>
                <button
                  ...
                  onClick={() => { setUnit("F"); setMenuOpen(false); }}
                >
                  Fahrenheit
                </button>
                <div className="h-px bg-white/10 my-1" />
        ...
        >
          ...
          <section className="page min-w-full px-5 flex flex-col">
            <div className="flex-1 flex items-center">
              {status === "ready" && data ? (
                <TodayHeadline deltaC={data.todayVsYesterdayC} />
              ) : status === "error" ? (
                <div className="text-[18px] leading-relaxed">
                  ...
                  <button
                    className="mt-4 border border-white/80 rounded-md px-4 py-2 text-[14px] hover:bg-white hover:text-black transition"
                    onClick={() => setCoords((c) => (c ? { ...c } : c))}
                  >
                    Retry
                  </button>
                </div>
              ) : (
                <div className="lw-muted text-sm">
          ...
          <section className="page min-w-full px-5 flex flex-col">
            <div className="flex-1 flex items-center">
              {status === "ready" && data ? (
                <TomorrowHeadline
                  deltaC={data.tomorrowVsTodayC}
                  code={data.tomorrowCode}
                />
              ) : (
                <div className="lw-muted text-sm">
        ...
        >
          {[0, 1].map((i) => (
            <button
              ...
              style={{ background: page === i ? "#e5e5e5" : "#3a3a3a" }}
            />
          ))}
        </div>
      </div>
